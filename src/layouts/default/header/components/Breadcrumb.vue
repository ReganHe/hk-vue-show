<template>
  <div :class="[prefixCls, `${prefixCls}--${theme}`]">
    <a-breadcrumb v-if="routes && routes.length" :routes="routes">
    ...
    <span v-else> {{ $route.meta.title || $route.name }} </span>˝
  </div>
</template>
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      watchEffect(async () => {
        ...
        routes.value = breadcrumbList.map((r) => {
          return {
            path: r.path,
            breadcrumbName: r.name,
          } as unknown as Route;
        });
      });
